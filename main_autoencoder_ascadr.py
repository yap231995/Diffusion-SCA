...
from src.utils import aes_label_cpa, cpa_method, aes_label_cpa_ascad
...
for r_xp in range(rep):
    ...
    dim_len_entr = random.randrange(2,7)      
    batch_entr = random.randrange(64,2049,32)  
    emb_entr = random.randrange(128,trace_size_original,32)  
    epoch_entr = random.randrange(40,101)     
    lr_entr = random.randrange(-5,-2)           

    dims = []
    for i in range((dim_len_entr)):
        dims.append(random.randrange(32,1025,32)) # random assignment of number of nodes

    embedding_size = copy.deepcopy(emb_entr)
    
    ae = Autoencoder(trace_size_original, embedding_size, dims).to(device)
    
    batch_size_ae = copy.deepcopy(batch_entr) 
    dataloaders_ae = {"train": torch.utils.data.DataLoader(dataloadertrain, batch_size=batch_size_ae,
                                shuffle=True, num_workers=num_workers)}
    epochs_ae = copy.deepcopy(epoch_entr) 
    lr_ae = 10**copy.deepcopy(lr_entr)
    ...
    if r_xp == 0:
        ...
        plt.savefig(image_root + 'Orig_traces_' + dataset + "_" + leakage + "_ascadr.png")


    fig, ax = plt.subplots(figsize=(15, 7))
    used_traces = dataloadertrain.X_profiling.shape[0]
    ...
    if r_xp == 0:
        ...
        label_correct_key = dataloadertrain.Y_profiling
        cpa_k = cpa_method(total_samplept, total_num_gen_trace,  label_correct_key[:used_traces],  real_traces)
        
        x_axis = [i for i in range(total_samplept)]
        ax.plot(x_axis, cpa_k, c="red", label = "$Sbox(pt\oplus k) \oplus r$")
        
        label_correct_key = dataloadertrain.plt_profiling[:,1]
        cpa_k = cpa_method(total_samplept, total_num_gen_trace,  label_correct_key[:used_traces],  real_traces)
        ax.plot(x_axis, cpa_k, c="blue",label = "$r$")
        ax.set_xlabel('Sample points', fontsize=20)
        ...
        plt.savefig(image_root + 'Actual_corr_' + dataset + "_" + leakage + "_ascadr.png")
    
    fig, ax = plt.subplots(figsize=(15, 7))
    
    label_correct_key = dataloadertrain.Y_profiling
    cpa_k = cpa_method(total_samplept, total_num_gen_trace,  label_correct_key[:used_traces],  reconstructed_traces[:used_traces,:])
    ...
    if len(nan_check)==len(cpa_k):
        ...
        used_d = copy.deepcopy(used_traces)
        ...
        if max_metric > best_corr:
            ...
            plt.savefig(image_root + 'Corr_' + dataset + "_" + leakage+  "_ascadr.png")
            ...
            plt.savefig(image_root + 'Gen_traces_' + dataset + "_" + leakage+ "_ascadr.png")
...
best_params = dict(dims=best_dim, batch_size=best_param[0], epoch_size=best_param[1], learn_rate=best_param[2], embedding_size=best_param[3], max_cpa=best_corr, key_rank=0)
