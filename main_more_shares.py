...
dataloadertrain = Custom_Dataset(root = root, dataset = dataset, leakage = leakage,embedding_size = embedding_size, transform =  transforms.Compose([ ToTensor_trace() ]))
...
with torch.no_grad():
    ...
    if masking_order == 0:
        ...
        new_masks = np.concatenate((new_masks, masking1, masking2, masking3), axis = 1) #This should include all masking.
        print("new_masks: ", new_masks.shape)
    #Starts sampling.
    if sampling == True:
        clip_denoised= False
...
if print_traces == True:
    ...
    plt.savefig(image_root + 'Generated_traces_' + dataset + "_" + leakage+"_epochs_"+str(epochs) + "_more_shares.png")
...
if cal_cpa == True:
    ...
    for k in range(256):
        print("key: ", k)
        label_k = aes_label_cpa_mask(dataset, new_plain, k, leakage,new_masks, order = masking_order) #new_plain is not used in AES_HD_ext
    ...
    plt.savefig(image_root + 'CPA_generated_' + dataset + "_" + leakage + "_epochs_" + str(epochs) + "_more_shares_2.png")
    plt.show()
