...
# for parameter search, observe the MSE...
batch_size = 100
epochs = 145
lr = 0.005

print_orig_cpa = False
...
save_root2 = root + 'Result_ASCADf/' + dataset2 + '_' + leakage +'/'
...
#mask_profiling = np.load(path_dataset + "mask_profiling.npy")
#mask_attack = np.load(path_dataset + "mask_attack.npy")
...
if dataset == "simulated_traces_order_0" or dataset == "Chipwhisperer":
    masking_order = 0
elif dataset == "simulated_traces_order_1" or dataset == "AES_HD_ext_plaintext" or dataset == "AES_HD_ext_sbox" or dataset == "AES_HD_ext_label" or dataset == "AES_HD_ext" or dataset == "ASCAD":
    masking_order = 1
...
print("masking order: ", masking_order)
...
if cal_cpa == True:
    ...
    if dataset == "AES_HD_ext_plaintext" or dataset == "AES_HD_ext_sbox" or dataset == "AES_HD_ext_label" or dataset == "AES_HD_ext":
        ...
        for shares in range(masking_order+1):
            ...
            if shares == 0:
                ax.plot(x_axis, cpa_k_m, c="blue", label = "$Sbox(pt\oplus k) \oplus r$")
            elif shares == 1:
                ax.plot(x_axis, cpa_k_m, c="green", label = "$r$")
            elif shares == 2:
