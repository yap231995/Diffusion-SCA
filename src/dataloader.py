...
from src.utils import load_chipwhisperer, generate_traces, calculate_HW, load_latent_dataset, load_ascad
import torch
...
class Custom_Dataset(Dataset):
    def __init__(self, root='./', dataset="Chipwhisperer", leakage="HW", transform=None, embedding_size=None):
        ...
        if dataset == 'simulated_traces_order_0' or dataset == 'simulated_traces_order_1' or dataset == 'simulated_traces_order_2' or dataset == 'simulated_traces_order_3':
            ...
            if save_data == True:
                self.X_profiling, self.Y_profiling, self.plt_profiling, self.masking_profiling = generate_traces(
                    n_traces=14000, n_features=trace_size, order=self.order)
                self.X_attack, self.Y_attack, self.plt_attack, self.masking_attack = generate_traces(n_traces=30000,
                                                                                                     n_features=trace_size,
                                                                                                     order=self.order)
                ...
                if os.path.exists(data_root + "masking_attack.npy") and os.path.exists(
                        data_root + "masking_profiling.npy"):
            ...
            self.plt_profiling, self.plt_attack), self.correct_key = load_chipwhisperer(root + data_root + '/',
            ...
            (self.X_profiling, self.X_attack), (self.Y_profiling, self.Y_attack), (
            self.plt_profiling, self.plt_attack), self.correct_key = load_ascad_variable(
                root + data_root, leakage_model=leakage, byte=byte, train_begin=0, train_end=45000, test_begin=0,
                test_end=5000)
            self.X_profiling_orig = self.X_profiling
            self.X_attack_orig = self.X_attack
    ...
    def choose_phase(self, phase):
        if phase == 'train':
            ...
            self.X, self.Y, self.Plaintext = np.expand_dims(self.latent_traces_profiling,
                                                            1), self.Y_profiling, self.plt_profiling
        elif phase == 'test_latent':
            self.X, self.Y, self.Plaintext = np.expand_dims(self.latent_traces_attack,
                                                            1), self.Y_attack, self.plt_attack
...
class Custom_Dataset_Profiling(Dataset):
    def __init__(self, nb_traces_attacks, root='./', dataset="Chipwhisperer", leakage="HW", transform=None,
                 embedding_size=None):
    ...
    def choose_phase(self, phase):
        if phase == 'train_label':
            self.X, self.Y, self.Plaintext = np.expand_dims(self.X_profiling_train,
                                                            1), self.Y_profiling_train, self.Y_profiling_train
        elif phase == 'validation_label':
            self.X, self.Y, self.Plaintext = np.expand_dims(self.X_profiling_val,
                                                            1), self.Y_profiling_val, self.Y_profiling_val
        elif phase == 'test_label':
            self.X, self.Y, self.Plaintext = np.expand_dims(self.X_attack, 1), self.Y_attack, self.Y_attack
...
class ToTensor_trace(object):
    ...
    def __call__(self, sample):
        ...
        return torch.from_numpy(trace).float(), torch.from_numpy(np.array(label)).long(), torch.from_numpy(
            np.array(plaintext)).long()
