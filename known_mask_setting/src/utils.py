...
'''
def obtain_plaintext(all_masks, correct_key):
    # print(all_masks)
    ...
    for i in range(getting_plain.shape[0]):
        getting_plain[i] = AES_Sbox[getting_plain[i]]
...
def obtain_plaintext(dataset,all_masks, correct_key):
    ...
    if dataset == "AES_HD_ext_plaintext":
        ...
        return getting_plain
    elif dataset == "AES_HD_ext_label" or dataset == "AES_HD_ext":  # [sbox_inv(pt_15^k)^pt_11,pt_11]
...
'''
def aes_label_cpa_mask(plt, correct_key, leakage, mask, order = 1):
...
def aes_label_cpa_mask(dataset, plt, correct_key, leakage, mask, order = 1):
    ...
    if order == 0:
        ...
        if dataset == "AES_HD_ext_plaintext":
        ...
        elif dataset == "AES_HD_ext_sbox" or dataset == "AES_HD_ext_label" or dataset == "AES_HD_ext" or dataset == "AES_HD_ext_AE":
            pt_15 = plt
            pt_11 = mask[:, 1]
            for i in range(num_traces):
                if leakage == 'HW':
            ...
            mask1 = mask[:, 1] # modify from 0 originally, column 0 is label, column 1 is mask
            for i in range(num_traces):
                if leakage == 'HW':
                    labels_for_snr[i] = HW(AES_Sbox[plt[i] ^ correct_key]^mask1[i])
                elif leakage == 'ID':
...
def aes_label_cpa_AES_HD(plt, correct_key, leakage):
    ...
    for i in range(num_traces):
        if leakage == 'HW':
            ...
            labels_for_snr[i] = AES_Sbox_inv[correct_key ^ int(plt[i, 15])] ^ plt[i, 11]
    return labels_for_snr

def aes_label_cpa_AES_HD_byte0(plt, correct_key, leakage):
    num_traces = plt.shape[0]
    labels_for_snr = np.zeros(num_traces)
    for i in range(num_traces):
        if leakage == 'HW':
            labels_for_snr[i] = HW(AES_Sbox_inv[correct_key ^ int(plt[i, 0])] ^ plt[i, 0])
        elif leakage == 'ID':
            #labels_for_snr[i] = AES_Sbox_inv[correct_key[15] ^ int(plt[i, 15])] ^ plt[i, 11]
            labels_for_snr[i] = AES_Sbox_inv[correct_key ^ int(plt[i, 0])] ^ plt[i, 0]
    return labels_for_snr

'''
def aes_label_cpa_ascad(plt, mask, correct_key, leakage):
    num_traces = plt.shape[0]
    labels_for_snr = np.zeros(num_traces)
    for i in range(num_traces):
...
def cpa_method(total_samplept, number_of_traces, label, traces):
    if total_samplept == 1:
        ...
        for t in tqdm(range(total_samplept), disable=True):
            cpa[t] = abs(np.corrcoef(label[:number_of_traces], traces[:number_of_traces, t])[1, 0])
        
        ##h = label[:number_of_traces]
        ##if len(h.shape) == 1:
        ##    h = h[..., np.newaxis]
        
        ##h = np.hstack([traces[:number_of_traces, :],h])
        ##cpa = np.corrcoef(h,rowvar=False)[-1,:-1]
    return np.abs(cpa)

'''
def calc_cpa(h,traces):
    # Calculate CPA between intermediate values and traces
    # Return c: array of correlation for each time samples
    
    if len(h.shape) == 1:
        h = h[..., np.newaxis]
        
    h = np.hstack([traces,h])
    c = np.corrcoef(h,rowvar=False)[-1,:-1]
    return abs(c)
'''
...
def load_aes_hd_ext_id2(aes_hd_database_file, leakage_model='HW',train_begin = 0, train_end = 45000,test_begin = 0, test_end = 5000):
    in_file = h5py.File(aes_hd_database_file, "r")
    last_round_key = [0xd0, 0x14, 0xf9, 0xa8, 0xc9, 0xee, 0x25, 0x89, 0xe1, 0x3f, 0x0c, 0xc8, 0xb6, 0x63, 0x0c, 0xa6]
    X_profiling = np.array(in_file['Profiling_traces/traces'])
    C_profiling = np.array(in_file['Profiling_traces/metadata'][:]['ciphertext'], dtype='uint8')
    ...
    print("Load Y_profiling")
    for i in tqdm(range(C_profiling.shape[0])):
        Y_profiling[i] = AES_Sbox_inv[last_round_key[0] ^ int(C_profiling[i, 0])] ^ C_profiling[i, 0]
    if leakage_model == 'HW':
        Y_profiling = np.array(calculate_HW(Y_profiling))
    ...
    print("Load Y_attack ")
    for i in tqdm(range(C_attack.shape[0])):
        Y_attack[i] = AES_Sbox_inv[last_round_key[0] ^ int(C_attack[i, 0])] ^ C_attack[i, 0]
    if leakage_model == 'HW':
        Y_attack = np.array(calculate_HW(Y_attack))
    print("X_attack : ", X_attack.shape)
    print("C_attack : ", C_attack.shape)
    print("Y_attack : ", Y_attack.shape)
    attack_key = last_round_key[0]
    print("key: ", attack_key)
    return (X_profiling[train_begin:train_end], X_attack[test_begin:test_end]), (Y_profiling[train_begin:train_end],  Y_attack[test_begin:test_end]),\
           (C_profiling[train_begin:train_end],  C_attack[test_begin:test_end]), attack_key
...
def load_ascad(ascad_database_file, leakage_model='HW', byte = 2,train_begin = 0, train_end = 45000,test_begin = 0, test_end = 5000):
    ...
    M_profiling = np.array(in_file['Profiling_traces/metadata'][:]['masks'][:, 15])
    if byte != 2:
    ...
    M_attack = np.array(in_file['Attack_traces/metadata'][:]['masks'][:, 15])
    attack_key = np.array(in_file['Attack_traces/metadata'][:]['key'][0, byte]) #Get the real key here (note that attack key are fixed)
    profiling_key = np.array(in_file['Profiling_traces/metadata'][:]['key'][0, byte]) #Get the real key here (note that attack key are fixed) => NEED TO MODIFY THIS FOR ASCAD VARIABLE?
    #print(attack_key)
    #print(profiling_key)
    if byte != 2:
    ...
    print("correct key (profiling):", profiling_key)
    print("correct key (attack):", attack_key)
    print()

    A = np.concatenate((np.expand_dims(P_profiling[train_begin:train_end], axis=1), np.expand_dims(M_profiling[train_begin:train_end], axis=1)), axis=1)
    B = np.concatenate((np.expand_dims(P_attack[test_begin:test_end], axis=1), np.expand_dims(M_attack[test_begin:test_end], axis=1)), axis=1)
    #return (X_profiling[train_begin:train_end], X_attack[test_begin:test_end]), (Y_profiling[train_begin:train_end],  Y_attack[test_begin:test_end]),\
    #       (P_profiling[train_begin:train_end],  P_attack[test_begin:test_end]), attack_key
    return (X_profiling[train_begin:train_end], X_attack[test_begin:test_end]), (Y_profiling[train_begin:train_end],  Y_attack[test_begin:test_end]),\
           (A,  B), attack_key
...
def load_ascad(ascad_database_file, leakage_model='HW', byte = 2,train_begin = 0, train_end = 45000,test_begin = 0, test_end = 5000):
    ...
    Y_profiling =  np.zeros(P_profiling.shape[0], dtype='uint8')
    print("Load Y_profiling")
    for i in tqdm(range(P_profiling.shape[0])):
        Y_profiling[i] = AES_Sbox[attack_key ^ int(P_profiling[i])] ^ M_profiling[i]
    if leakage_model == 'HW':
        Y_profiling = np.array(calculate_HW(Y_profiling))
    Y_attack =  np.zeros(P_attack.shape[0], dtype='uint8')
    print("Load Y_profiling")
    for i in tqdm(range(P_attack.shape[0])):
        Y_attack[i] = AES_Sbox[attack_key ^ int(P_attack[i])] ^ M_attack[i]
    if leakage_model == 'HW':
        Y_attack = np.array(calculate_HW(Y_attack))
    ...
    if leakage_model == 'HW':
        ...
        print("Y_profiling total shape", Y_profiling.shape)
    print("P_profiling total shape", A.shape)
    print("X_attack total shape", X_attack.shape)
    if leakage_model == 'HW':
        print("Y_attack total shape", len(Y_attack))
    else:
        print("Y_attack total shape", Y_attack.shape)
    print("P_attack total shape", B.shape)
    print("correct key (profiling):", profiling_key)
    print("correct key (attack):", attack_key)
    print()
    #return (X_profiling[train_begin:train_end], X_attack[test_begin:test_end]), (Y_profiling[train_begin:train_end],  Y_attack[test_begin:test_end]),\
...
def load_ascad_variable(ascad_database_file, leakage_model='HW', byte = 2,train_begin = 0, train_end = 45000,test_begin = 0, test_end = 5000):
    print(ascad_database_file)
    in_file = h5py.File(ascad_database_file, "r")
    X_profiling = np.array(in_file['Profiling_traces/traces'])
    X_profiling = X_profiling.reshape((X_profiling.shape[0], X_profiling.shape[1]))
    ...
    P_profiling = np.array(in_file['Profiling_traces/metadata'][:]['plaintext'][:, byte])
    M_profiling = np.array(in_file['Profiling_traces/metadata'][:]['masks'][:, 17])
    if byte != 2:
        key_profiling = np.array(in_file['Profiling_traces/metadata'][:]['key'][:,byte])
        Y_profiling = np.zeros(P_profiling.shape[0])
        print("Loading Y_profiling")
    ...
    P_attack = np.array(in_file['Attack_traces/metadata'][:]['plaintext'][:, byte])
    M_attack = np.array(in_file['Attack_traces/metadata'][:]['masks'][:, 17])
    attack_key = np.array(in_file['Attack_traces/metadata'][:]['key'][0, byte]) #Get the real key here (note that attack key are fixed)
    profiling_key = np.array(in_file['Profiling_traces/metadata'][:]['key'][:, byte])
    #print(attack_key)
    print(profiling_key)
    if byte != 2:
        print("Loading Y_attack")
    ...
    print("Load Y_profiling")
    for i in tqdm(range(P_profiling.shape[0])):
        Y_profiling[i] = AES_Sbox[profiling_key[i] ^ int(P_profiling[i])] ^ M_profiling[i]
    if leakage_model == 'HW':
        Y_profiling = np.array(calculate_HW(Y_profiling))
    Y_attack =  np.zeros(P_attack.shape[0], dtype='uint8')
    print("Load Y_attack")
...
def rank_compute(prediction, att_plt, correct_key,leakage, dataset):
    ...
    for i in range(nb_traces):
        for k in range(256):
            if dataset == "AES_HD_ext" or dataset == "AES_HD_ext_AE":
                if leakage == 'ID':
            ...
            elif dataset == "AES_HD_ext_ID" or dataset == "AES_HD_ext_AE_ID":
                if leakage == 'ID':
